...
bool GetMacAddr(
  MacAddr* mac, const std::string& iface_path, sdbusplus::bus::BusAbc &dbus)
{
  if (mac == nullptr)
  {
    std::cerr << "GetMacAddr MAC Address is nullptr" << std::endl;
    return false;
  }
  ...
  {
    std::cerr << "GetMacAddr Failed to get MAC Address" << std::endl;
    return false;
  }
  ...
  {
    std::cerr << "GetMacAddr Failed to parse MAC Address: "
              << mac_addr
              << std::endl;
    return false;
  }
